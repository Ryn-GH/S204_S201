...
from flask_cors import CORS # Nécessaire pour permettre les requêtes depuis le frontend (CORS)
...
def get_departements():
    ...
    departements_db = conn.execute('SELECT code_departement, nom_departement FROM Département ORDER BY nom_departement').fetchall()
    conn.close()
    departements = [{'code': d['code_departement'], 'nom': d['nom_departement']} for d in departements_db]
...
def get_parametres():
    ...
    parametres = [{
        ...
        'unite': p['libelle_unite'] # Si disponible, sinon gérer le cas où c'est null
